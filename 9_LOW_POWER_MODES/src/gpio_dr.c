...
void Gpio_Init(GPIO_TypeDef *PORT, uint8_t pin, uint8_t MODE, uint8_t OUTPUT_TYPE, uint8_t PUPD)
{
    PORT->MODER &= ~(3<<(pin*2));           // clear pin 
    PORT->MODER |= ((MODE)<<(pin*2));       // Set the PIN mode
    PORT->OTYPER &= ~(1<<pin);              // Clear Output Type
...
void GPIO_UART_Init(void){
    Gpio_Init(GPIOA,2,AF,OPEN_DRAIN,NO_PUPD);   // PA2 - TX
    Gpio_Af(GPIOA,2,AF1);                       // AF1
    Gpio_Init(GPIOA,3,AF,OPEN_DRAIN,NO_PUPD);   // PA3 - RX
    Gpio_Af(GPIOA,3,AF1);                       // AF1
}