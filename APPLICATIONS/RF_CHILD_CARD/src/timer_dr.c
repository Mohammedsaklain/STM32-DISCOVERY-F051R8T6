...
void Timer_Input_Capture(uint8_t timer_no)//, uint8_t ch_no)
{
    TIM_TypeDef * TIMER = Get_Timer_No(timer_no);
    TIMER->CCMR1 &= (3<<8);                     // Clear Bits at (9-8)
    TIMER->CCMR1 |= TIM_CCMR1_CC2S_0;           // Configure for Input Capture mode
    TIMER->CCER  |= TIM_CCER_CC2E;              // Input Capture Enabled for CH1
    TIMER->CCER  |= (TIM_CCER_CC2P | TIM_CCER_CC2NP);  //Capture mode for both edges, rising & falling
    TIMER->CR1 |= TIM_CR1_CEN; 
}
...
{
    TIM_TypeDef * TIMER = Get_Timer_No(timer_no);
    while(!((TIMER->SR)&(TIM_SR_CC2IF))){}	// wait till rising & falling edge pulse is present
	TIMER->SR &= ~(1<<0);
	CURRENT_COUNT = TIMER->CCR2;
