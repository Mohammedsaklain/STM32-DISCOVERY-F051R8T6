...
uint8_t Flash_Memory_Unlock(void){
    ...
    status = Flash_Status();
    if(status!=FLASH_OK){
        MS_printf("ERROR: Not ready to unlock Flash\r\n");
    }
...
uint8_t Flash_Memory_Erase_Pages(uint32_t start_page_addr,uint32_t num_pages){    
    ...
    if(status!=FLASH_UNLOCKED){
        MS_printf("ERROR: FLASH Not UNLOCKED\r\n");
        return status;
    }
    ...
    for(uint8_t i=0;i<num_pages;i++){
        ...
        if(status!=FLASH_OK){
            MS_printf("ERROR: Page erase failed at 0x%X.\r\n",(unsigned int)current_page_addr);
...
uint8_t Flash_Program_Halfword(uint32_t Address, uint16_t Data){
    ...
    if ((Address % 2) != 0) {
        MS_printf("ERROR: Address not half-word aligned (0x%X).\r\n", (unsigned int)Address);
        return FLASH_ERROR;
    }

    status = Flash_Memory_Unlock();
    if(status!=FLASH_UNLOCKED){
        MS_printf("ERROR: Flash not unlocked for program.\r\n");
        return status;
    }

    status = Flash_Status();
    if(status != FLASH_OK) {
        MS_printf("ERROR: Flash not ready for program\r\n");
        Flash_Memory_Lock();
        return status;
    }
    ...
    *(volatile uint16_t *)Address = Data;       // Write Data to target address
    ...
    status = Flash_Status();
    if (status != FLASH_OK) {
        MS_printf("ERROR: Program failed at 0x%X.\r\n", (unsigned int)Address);
    }
...
uint16_t Flash_Read_Halfword(uint32_t address) {
    ...
    if ((address % 2) != 0) {
        ...
        MS_printf("Warning: Reading unaligned half-word at 0x%X. Might return unexpected data.\r\n", (unsigned int)address);
    }
...
uint32_t Flash_Read_Word(uint32_t address) {
    ...
    if ((address % 4) != 0) {
        ...
        MS_printf("Warning: Reading unaligned word at 0x%X. Might return unexpected data.\r\n", (unsigned int)address);
    }
